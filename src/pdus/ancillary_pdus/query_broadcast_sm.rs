...
impl QueryBroadcastSmRequest {
    ...
    pub fn encode(&self, writer: &mut impl Write) -> Result<(), PduError> {
        #[cfg(feature = "tracing")]
        tracing::debug!("Encoding QueryBroadcastSmRequest");
    ...
    pub fn decode(buffer: &[u8]) -> Result<Self, PduError> {
        #[cfg(feature = "tracing")]
        tracing::debug!(
            "Decoding QueryBroadcastSmRequest from {} bytes",
            buffer.len()
        );
...
impl QueryBroadcastSmResponse {
    ...
    pub fn encode(&self, writer: &mut impl Write) -> Result<(), PduError> {
        #[cfg(feature = "tracing")]
        tracing::debug!("Encoding QueryBroadcastSmResponse");
        let tlvs_len: usize = self
            .optional_params
            .iter()
            .map(|tlv| 4 + tlv.length as usize)
    ...
    pub fn decode(buffer: &[u8]) -> Result<Self, PduError> {
        #[cfg(feature = "tracing")]
        tracing::debug!(
            "Decoding QueryBroadcastSmResponse from {} bytes",
            buffer.len()
        );
        if buffer.len() < HEADER_LEN {
            return Err(PduError::BufferTooShort);
